...
If `$HERENOW_API_KEY` or `~/.herenow/credentials` exists, use it. The
publish script already loads that key. Do not ask for an email and do
not create an account.

If no credentials exist, publish anyway. An anonymous 24h site is fine.
Do not start a sign-in flow on your own. If the user asks to authenticate,
follow https://here.now/docs and ask which email to use.

Drive and workspace publishes need a key. If none exists, say so and
skip those instead of signing in.
...
A saved API key makes the site permanent on that account. Without one,
the site is anonymous and expires in 24 hours.
...
- When `publish_result.auth_mode=anonymous`: tell the user the site is temporary (24h) and not saved to an account.
